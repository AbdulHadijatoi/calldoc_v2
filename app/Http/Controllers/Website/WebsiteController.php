...
class WebsiteController extends Controller
{
    ...
    {
        $setting = Setting::first();
        $doctor = Doctor::with(['category', 'expertise','user'])->where("slug", $slug)->first();
        if(!$doctor){
            return redirect()->back()->withErrors('Doctor not found');
        }
        $id = $doctor->id;
        $doctor['is_fav'] = $this->checkFavourite($doctor['id']);
        ...
        $work_hours_data =  WorkingHour::where([['doctor_id', $id], ['day_index', Carbon::now(env('timezone'))->format('l')]])->first();
        if($work_hours_data){

            $today_date[] = $work_hours_data->period_list;
        }
        $todayData = WorkingHour::where([['doctor_id', $id], ['day_index', Carbon::now(env('timezone'))->format('l')]])->first();
        $today = null;
        if($todayData){
            $today = $todayData->status;
        }
        $currently_open = 0;
        if(isset($today_date[1])){
